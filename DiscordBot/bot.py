...
#    return ctx.channel.id == channelID
...
async def help(ctx, command = None):
    ...
    if command == 'backup':
        await ctx.send('``backup [save/load]`` - Load or saves the topic and notes from last backup. Auto-backups are made when adding or editing a note or adding a topic. You can load from backup if you accidentally delete any topics or notes.')
        check = True
    if check == False:
...
async def backup(ctx, *, msg = None):
    if msg == 'save':
        ...
        if load[0] == False:
            ...
            await ctx.send('Succesfully loaded topics and notes from backup.')
    else:
        await ctx.send('Please input save or load ``backup [save/load]``')
